...
<head>
  ...
  <!-- <script src="{$host}zb_users/theme/yt_blog/style/js/jquery.pjax.2017.js" type="text/javascript"></script> -->
...
<body xmlns="http://www.w3.org/2000/svg">
  <div id="fixed-content-holder">
    <div id="float-top-navigation" class="flex-layout-row">
      <div class="navigation-icon">
        <a class="m-pjax" href="{$zbp->host}" title="回到主页">
          <svg id="webicon">
            <path class="icon-color icon-yt"
              d="M41,11v8H39V9h2Zm5,12H42V19h2v2h2Zm3,0V39H47V21h2Zm5-2v2H50V21h2V19h2Zm3-10v8H55V9h2Z" />
            <path class="icon-color icon-home"
      ...
      <div class="navigation-control flex-layout-row">
        <div class="navigation-item navigation-label">
          <span class="navigation-item-content flex-layout-colum">
          ...
          <div class="navigation-dropdown">
            ...
            <a href="{$Catenew.Url}" class="m-pjax navigation-dropdown-item"><span>{$Catenew.Name}</span></a>
            {/foreach}
          </div>
        </div>
        <div class="navigation-item navigation-label">
          ...
          <div class="navigation-dropdown">
            ...
            <a href="{$tag.Url}" title="{$tag.Name} Tag" class="m-pjax navigation-dropdown-item"><span>{$tag.Name}
                ({$tag.Count})</span></a>
            {/foreach}
          </div>
        </div>
        <a class="m-pjax navigation-item navigation-label" href="{$host}?id=2">
          <span class="navigation-item-content flex-layout-colum">
            <span class="icon-holder segoe">&#xed1e;</span>
      ...
      <div class="navigation-fixed flex-layout-row">
        <div class="navigation-search-holder">
          <form role="search" name="search" method="post" action="{$host}zb_system/cmd.php?act=search">
            <input id="edtSearch" class="navigation-search navigation-search-input" type="text" autocomplete="off"
              value="" name="q" placeholder="请输入关键字搜索">
            <input type="submit" class="navigation-search navigation-search-submit segoe" value="&#xE721;" title="搜索">
          </form>
        </div>
        <a class="navigation-login-holder flex-layout-row" href="{$zbp->host}zb_system/login.php" title="登录">
          {if $user.ID>0}
          <div class="navigation-login-usericon"
            style="background-image: url({$zbp->host}zb_users/avatar/{$user.ID-1}.png);"></div>
          {else}
          <span class="navigation-login segoe">&#xe13d;</span>
          {/if}
        </a>
      </div>
    </div>
    <div id="fixed-top-banner">
      ...
      <div class="banner-img banner-imglayout" style="background-image: url({$zbp->Config('yt_blog')->bannerurl});background-position:{$banneralign[0]} {$banneralign[1]}"></div>
