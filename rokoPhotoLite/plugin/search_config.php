...
function rokoPhotoLite_SearchMain() {
	global $zbp;
...
if ($articles){
	foreach($articles as $article){
		$intro = preg_replace('/[\r\n\s]+/', '', trim(rokoPhotoLite_SubStrStartUTF8(TransferHTML($article->Content,'[nohtml]'),$q,150)) . '...');
		$article->Intro = str_ireplace($q,$qc,$intro);
		$article->Title = str_ireplace($q,$qc,$article->Title);
	}
		$zbp->template->SetTags('rokoPhotoLiteSearchSubtitle','');
}else{
